...
>>>>>>> 3365a1a (Fix map legend and update visualization logic)
    ...
    # don't add duplicate traces if they already exist
    existing_names = {t.name for t in fig.data if getattr(t, "name", None)}

    # 1) Steel Plants (purple)
    if "plants" in globals() and isinstance(plants, (list, tuple)) is False:
        latc, lonc = _find_lat_lon(plants)
        if latc and lonc and "Steel Plants" not in existing_names:
            try:
                fig.add_trace(go.Scattermapbox(
                    ...
                    name="Steel Plants",
                    hoverinfo="text",
                    hovertext=plants.apply(lambda r: f"{r.get('name','') or r.get('Plant','')}\n{r.get('State', r.get('state',''))}", axis=1)
                ))
                existing_names.add("Steel Plants")
            except Exception:
                pass

    # 2) Steel Plants with BF (red) — try to detect a bf column, otherwise skip if absent
    if "plants" in globals() and isinstance(plants, (list, tuple)) is False:
        bf_col = None
        for candidate in ["has_bf", "bf", "blast_furnace", "has_blast_furnace"]:
            if candidate in [c.lower() for c in plants.columns]:
                # find exact case column name
                bf_col = plants.columns[[c.lower() for c in plants.columns].index(candidate)]
                break
        if bf_col is not None and "Steel Plants with BF" not in existing_names:
            try:
                bf_df = plants[plants[bf_col].astype(bool)]
    ...
    # 3) Geocoded companies (green) — common DF name: 'companies' or 'geocoded_companies'
    for dfname, legend_name, color in [
        ("companies", "Geocoded Companies", "rgb(0,128,0)"),
        ("geocoded_companies", "Geocoded Companies", "rgb(0,128,0)")
    ]:
        if dfname in globals() and dfname in globals() and legend_name not in existing_names:
            df = globals()[dfname]
    ...
    # 4) Rice mills (orange) — common DF name: 'rice_mills' or 'rice_mill'
    for dfname, legend_name, color in [
